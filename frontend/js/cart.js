...
if (storage.has('products')) {     
    ...
    // Affichage de l'encart 'Récapitulatif'.
    displayHTML('productNumber', countArticles());
    displayHTML('totalCart', money(total));
    displayHTML('totalPrice', money(grandTotal));    

    // On écoute les éventuelles modifications du panier (suppression de produit(s), modification de quantité(s)).
    products.forEach((product) => {
        listenForDeletion(product._id);        
        listenForChangeQuantity(product._id);
    ...
    listenRemoveAll();
    listenForValidation();
    listenForCartSubmission(grandTotal);
...
function listenForChangeQuantity(productId) {
    getElement(`inputQuantity-${productId}`).addEventListener('change', (input) => {
        products.forEach((item) => {
            if (item._id === productId) {
                item.quantity = input.target.valueAsNumber;
            }
        });
